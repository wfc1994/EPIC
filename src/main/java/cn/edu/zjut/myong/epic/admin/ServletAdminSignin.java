 ...
 */
public class ServletAdminSignin extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            String signingName = request.getParameter("name").trim();
            String signingPassword = request.getParameter("password").trim();
            String code = AdminStation.signin(signingName, signingPassword);

            if (code != null) {
                json.add("login", true);
                json.add("code", code);
            } else {
                json.add("login", false);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
