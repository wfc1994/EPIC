 ...
 */
public class ServletAdminListImages extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            Map<String, String> imgs = AdminStation.getExpImages();
            for (String name : imgs.keySet()) {
                JsonObjectBuilder iobj = Json.createObjectBuilder();
                iobj.add("name", name);
                iobj.add("url", imgs.get(name));
                json.add(iobj);
            }
        } catch (Exception e) {
            e.printStackTrace();
